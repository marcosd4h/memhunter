...
peconv::ALIGNED_BUF peconv::load_resource_data(OUT size_t &out_size, int res_id, const LPWSTR res_type, HMODULE hInstance)
{
    if (hInstance == nullptr) {
        hInstance = GetModuleHandleA(NULL);
    }
    HRSRC res = FindResourceW(hInstance, MAKEINTRESOURCE(res_id), res_type);
    if (!res) {
#ifdef _DEBUG

...
// ===============================================================================
// Code taken from https://github.com/taylorjg/DumpToken with slight modifications 
// ===============================================================================
...
namespace TokenUtils
{
	...
	{
		wchar_t formattedBuff[MAX_BUFFER_SIZE] = { 0 };

		va_list varadic;
		va_start(varadic, buffer);
		_vsnwprintf_s(formattedBuff, (MAX_BUFFER_SIZE - 1), MAX_BUFFER_SIZE, buffer, varadic);
		va_end(varadic);
...
#define CREATE_ENUM_TABLE_ENTRY(e) { (long) e, _T(#e) }
#define ENUM_TABLE_AND_SIZE(EnumTable) (EnumTable), (sizeof ((EnumTable)) / sizeof (*(EnumTable)))
	...
	bool GetBasicTokenInfo(HANDLE hProcess, std::wstring &output)
	{
		bool ret = false;
		HANDLE hToken = nullptr;
		
		//if (OpenThreadToken(hProcess, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, false, hToken))
		if (OpenProcessToken(hProcess, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken))
		{
			...
			{
				output.append(ManagedOutputBuffer::GetInstance().GetBuffer());
				ManagedOutputBuffer::GetInstance().Clear();
				ret = true;
			}

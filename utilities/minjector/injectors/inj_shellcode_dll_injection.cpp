#include "../common.h"
#include "inj_shellcode_dll_injection.h"

bool InjectorShellcodeDLL::Execute(const std::wstring &codeToInject, const std::wstring &targetToInject)
{
	bool ret = false;
			...
			if (InjectorHelpers::IsValidTargetPID(targetToInject))
			{
				...
				if (resultAllocation)
				{
						...
						(targetProc.memory().Write(bufferAddr, codeToInject.size() * sizeof(wchar_t), codeToInject.c_str()) == ERROR_SUCCESS))
					{
						...
						if (asmPtr && LoadLibraryWPtr && targetProc.valid())
						{
							...
							{
								std::wcout << L"[+] Success! DLL injected via InjectorShellcodeDLL method" << std::endl;
								ret = true;
